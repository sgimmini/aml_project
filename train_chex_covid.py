...
experiment_name = "chex_covid_pretrained_chexpert_swav_e950_trained_full"
pretrained_model = "converted_vissl_swav_covid_pretrained_chexpert_e950.torch"
...
# - init datasets
traindata = ImageFolder("chexpert_covid_ImageFolder/train", transform=compose)

ten_percent = int(len(traindata)*0.1)
traindata, _ = torch.utils.data.random_split(traindata, [ten_percent, len(traindata)-ten_percent], generator=torch.Generator().manual_seed(42))
...
for epoch in range(NUM_EPOCHS):
    ...
    train_total = 0
    train_correct = 0
    for data, label in tqdm(train_loader, desc="Train", leave=False, bar_format='{l_bar}{bar:20}{r_bar}{bar:-10b}'):
        data = data.cuda()
        label = label.cuda()
        out = network(data)
    ...
    valid_total = 0
    valid_correct = 0
    for data, label in tqdm(valid_loader, desc="Valid", leave=False, bar_format='{l_bar}{bar:20}{r_bar}{bar:-10b}'):
        network.eval()
        data = data.cuda()
        label = label.cuda()
        out = network(data)
